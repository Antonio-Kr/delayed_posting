...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<IUser> {
    ...
    let x = await this.userModel.update(
      { _id: userThis._id },
      { $set: { registerOk: 'pending' } },
    );
  ...
  async sendToken(email: string) {
    return await this.client
      .send<Promise<IJwtToken>, string>('tokenRegister', email)
      .toPromise();
  }
  ...
  async registerOk(user: IUser) {
    return await this.userModel.update(
      { _id: user._id },
      { $set: { registerOk: 'active' } },
    );
  }

  async tokenCkeck(email: string) {
    return await this.client
      .send<Promise<IJwtToken>, string>('tokenCheck', email)
      .toPromise();
  }
  ...
  async userUpdate(
    ...
    await this.userModel.update(
      { email: user.email },
      {
        $set: { firstName: firstName, lastName: lastName, timezone: timezone },
      },
    );
